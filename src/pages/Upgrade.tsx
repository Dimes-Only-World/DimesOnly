...
interface Package {
  ...
  warning?: string;
  benefits: string[];
  installmentCount?: number; // if >1, installment plan is allowed
...
const packages: Package[] = [
  {
    ...
    warning: "MALES AND NORMAL FEMALES",
    benefits: [
      "One Year of Flame Flix Subscription in Phase 6.",
      "10% Discount site wide for 1 year.",
      "Get Overrides from Strippers and Exotics",
  ...
  },
  {
    ...
    benefits: [
      ...
      "View X-Rated photos and videos of strippers and exotic females",
...
const UpgradePageInner: React.FC = () => {
  ...
  const handleUpgrade = async () => {
    ...
    const isInstallment =
      paymentOption === "installment" && selectedPackage?.installmentCount;
    const paymentAmount = isInstallment
    ...
    try {
      // 1. Insert membership upgrade record
      const upgradePayload = {
        user_id: userData!.id,
        upgrade_type: selectedPackage!.id,
        payment_amount: paymentAmount,
        payment_method: isInstallment ? "paypal_installment" : "paypal_full",
      ...
      // 2. Update phone number on user profile (fire & forget)
      await supabase
        .from("users")
        .update({ phone_number: phoneNumber })
        .eq("id", userData!.id);

      // 3. Create PayPal order via edge function
        ...
        await supabase.functions.invoke("create-paypal-order", {
          body: {
            ...
            description: `${selectedPackage!.name} – ${
              isInstallment ? "Installment 1" : "Full Payment"
            }`,
          },
        });
              ...
              {packages.map((pkg) => (
                ...
                >
                  <CardHeader>
                    ...
                    {pkg.warning && (
                      <p className="text-red-400 font-bold">{pkg.warning}</p>
                    )}
                    {pkg.subtitle && (
                    ...
                    {pkg.monthly && (
                      <p className="text-sm text-gray-300">{pkg.monthly}</p>
                    )}
                  </CardHeader>
                  <CardContent>
  ...
  // ----------------------------
  // UI – Checkout View for Selected Package
  // ----------------------------
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white">
      <div className="max-w-4xl mx-auto px-4 py-8">
        {/* Back */}
        <button
          className="flex items-center gap-2 text-sm mb-6 text-gray-400 hover:text-white"
          onClick={resetState}
        >
        ...
        <div className="text-center mb-12">
          ...
          {selectedPackage.monthly && (
            <p className="text-sm text-gray-400">{selectedPackage.monthly}</p>
          )}
        </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                >
                  <div className="text-center">
                    ...
                    <h3 className="text-xl font-bold text-white mb-2">
                      {selectedPackage.installmentCount} Installments
                    </h3>
                    <div className="text-lg font-bold text-pink-400 mb-1">
                      ${selectedPackage.installmentAmount?.toFixed(2)} each
                    </div>
                    <div className="text-sm text-gray-400 mb-2">
                      × {selectedPackage.installmentCount} payments
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="text-center">
          <Button
            ...
            className="bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700 text-black font-bold text-lg px-12 py-4 rounded-xl shadow-lg transform hover:scale-105 transition-all duration-200"
          >
            {upgradeInProgress
              ? "Processing..."
              : `Pay $${(paymentOption === "installment" &&
                selectedPackage.installmentAmount
                  ? selectedPackage.installmentAmount
                  : selectedPackage.price
...
// ----------------------------
// Wrapper – provides react-query client & auth guard
// ----------------------------
