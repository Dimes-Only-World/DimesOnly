      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {rows.map((c) => (
          <Card key={c.id}><CardContent className="p-3 space-y-2">
            <div className="aspect-video bg-muted rounded overflow-hidden">
              {c.media_type === "video"
                ? <video src={c.url} controls className="w-full h-full object-cover" / controlsList="nodownload" disablePictureInPicture disableRemotePlayback>
              ...
              {media.map((m) => (
                <div key={m.id} className="relative w-24 h-24 bg-muted rounded overflow-hidden">
                  {m.media_type === "photo" ? (
                    ...
                    <video src={m.url} className="w-full h-full object-cover" / controlsList="nodownload" disablePictureInPicture disableRemotePlayback>
                  )}
                  <button onClick={() => removeMedia(m)} className="absolute top-1 right-1 bg-black/60 rounded-full p-1">
                    <Trash2 className="w-3 h-3 text-white" />
                  </button>
