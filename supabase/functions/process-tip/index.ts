...
const SOLD_OUT_MESSAGE = "Jackpot is maxed out for the upcoming drawing. Tipping will resume at Saturday 12:00 am PST.";
...
serve(async (req)=>{
  ...
  try {
    ...
    const { tipper_id, tipper_username, tipped_username, amount, message, referrer_username, paypal_capture_id } = body;
    ...
    if (parsedAmount > MAX_TIP) {
      return new Response(JSON.stringify({
        error: `Maximum tip per transaction is $${MAX_TIP}.`
      }), {
        headers: corsHeaders,
        status: 400
      });
    ...
    let poolId = null;
    let poolStatus = null;
    let poolMaxTickets = null;
    let poolCurrentTickets = 0;
    if (ticketCount > 0) {
      try {
        const { data: poolView, error: poolErr } = await supabase.from("v_jackpot_active_pool").select("pool_id,status,max_tickets,sales_resume_at,guaranteed_draw").single();
        ...
        if (poolView) {
          ...
          poolMaxTickets = typeof poolView.max_tickets === "number" ? poolView.max_tickets : null;
        }
        if (!poolId) {
          const { data: fallback, error: fallbackErr } = await supabase.from("jackpot_pools").select("id,status,max_tickets").in("status", [
            "open",
            "sold_out"
          ]).order("created_at", {
            ascending: false
          ...
          if (fallbackErr && fallbackErr.code !== "PGRST116") {
            throw new Error(`Fallback pool lookup failed: ${fallbackErr.message}`);
          }
          if (fallback) {
            poolId = fallback.id ?? null;
            poolStatus = fallback.status ?? null;
            poolMaxTickets = typeof fallback.max_tickets === "number" ? fallback.max_tickets : null;
          }
        }
        if (poolId) {
          const { count, error: countErr } = await supabase.from("jackpot_tickets").select("id", {
            count: "exact",
            head: true
          }).eq("pool_id", poolId);
          if (countErr) {
          ...
          if (poolStatus === "sold_out" || poolMaxTickets !== null && poolCurrentTickets >= poolMaxTickets) {
            return new Response(JSON.stringify({
              error: SOLD_OUT_MESSAGE
            }), {
              headers: corsHeaders,
              status: 409
            });
          }
          if (poolMaxTickets !== null && poolCurrentTickets + ticketCount > poolMaxTickets) {
            return new Response(JSON.stringify({
              error: SOLD_OUT_MESSAGE
            }), {
              headers: corsHeaders,
        ...
        return new Response(JSON.stringify({
          error: "Jackpot ticket sales are unavailable right now. Please try again later."
        }), {
          headers: corsHeaders,
          status: 503
        });
    ...
    const { data: tippedUser, error: tippedErr } = await supabase.from("users").select("id").eq("username", tipped_username).single();
    if (tippedErr || !tippedUser) throw new Error("Tipped user not found");
    const performerShare = roundCurrency(parsedAmount * PERFORMER_RATE);
    const jackpotContribution = roundCurrency(parsedAmount * JACKPOT_RATE);
    let refCommission = referrer_username ? roundCurrency(parsedAmount * REFERRER_RATE) : 0;
    let companyShare = roundCurrency(parsedAmount - performerShare - jackpotContribution - refCommission);
    let refUserId = null;
    if (referrer_username && refCommission > 0) {
      const { data: refUser, error: refErr } = await supabase.from("users").select("id").eq("username", referrer_username).maybeSingle();
      if (refErr) {
      ...
      if (!refUserId) {
        refCommission = 0;
        companyShare = roundCurrency(parsedAmount - performerShare - jackpotContribution);
      }
    }
    const allocatedTotal = performerShare + jackpotContribution + refCommission + companyShare;
    ...
    // Basic tip row
    const { data: tipRow, error: tipErr } = await supabase.from("tips").insert({
    ...
    // Tip transaction history
      ...
      // Update weekly earnings aggregates
    ...
    if (!poolId && ticketCodes.length) {
      const { data: poolRow } = await supabase.from("jackpot_pools").select("id").eq("status", "open").order("created_at", {
        ascending: false
      }).limit(1).maybeSingle();
    ...
    if (poolId && ticketCodes.length) {
      ...
      if (jtErr) console.error("jackpot_tickets insert error", jtErr);
        ...
        if (newTotal >= poolMaxTickets) {
          ...
          let salesResumeAt = null;
          const { data: resumeData, error: resumeErr } = await supabase.rpc("jackpot_next_sales_start", {
            now_ts: nowIso
          });
          if (!resumeErr && resumeData) {
            salesResumeAt = resumeData;
          }
          const { error: poolUpdateErr } = await supabase.from("jackpot_pools").update({
            status: "sold_out",
            sold_out_at: nowIso,
    ...
    if (poolId) {
      const { data: poolAmountRow, error: poolAmountErr } = await supabase.from("jackpot_pools").select("current_amount").eq("id", poolId).maybeSingle();
      if (poolAmountErr) {
    ...
    const { error: ledgerErr } = await supabase.from("jackpot_ledger").insert({
      tip_id: tipRow.id,
      tipper_id,
    ...
    if (refUserId && refCommission > 0) {
      const { error: payoutErr } = await supabase.from("commission_payouts").insert({
        user_id: refUserId,
